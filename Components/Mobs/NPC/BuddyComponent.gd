...
func _ready() -> void:
	...
	if buddy_group and set_buddy_on_start:
		...
		for potential_buddy in potential_buddies:
			...
			if !buddy_component:
				continue
			if buddy_component.buddy_group != buddy_group or buddy_component.buddy:
				continue
			
			buddy = potential_buddy
			buddy_component.buddy = parent
	
	if set_buddy_as_attack_target and buddy:
		var attack_target_component: Node = parent.get_node_or_null("AttackTargetComponent")
		if !attack_target_component and parent.has_node("AI"):
...
func on_gibbed(emitter) -> void:
	if emitter != buddy:
		return
	
	var rage_component: Node = parent.get_node_or_null("RageComponent")
